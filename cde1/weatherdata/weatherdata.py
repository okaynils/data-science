...
to be running before using the functions defined in this file. This script requires
influxdb version 1.8 or lower and is not compatible with influxdb 2.0 and higher.
...
def __define_types(data, date_format):
    ...
    if not data.empty: #data is not empty
        ...
        data.set_index('timestamp', inplace = True) #set "timestamp" as the index column and delete the old index column

    data.replace('.', 0, inplace = True) #replace al the missing values (represented as .) with a 0
